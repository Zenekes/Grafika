...
void update_camera(Camera* camera, double time, double wheel_rotation, vec3 wheel_position)
{
    if (camera->locked) {
        ...
        double radius = 1.5; // Adjust this to set the distance away from the wheel.

        camera->position.x = wheel_position.x;
        camera->position.y = wheel_position.y + radius* -sin(angle);
        camera->position.z = wheel_position.z + radius* cos(angle);
